...
class FirstPersonAction extends BaseClass{
    ...
    public function checkAction(BaseClass $Base){
        ...
        $stmt = $this->_dbh->prepare("SELECT * FROM action WHERE map_id = $this->map AND status = 0");
        $stmt->execute();
    ...
    public function doAction(BaseClass $Base){error_log(print_r($Base, 1 ));
        $this->map = $Base->getMap_id();       error_log("this map :".$this->map);
        ...
        if(empty($_SESSION['inventaire'])){
            ...
            $stmt = $this->_dbh->prepare("SELECT * FROM action WHERE map_id = $this->map AND status = 0");
            $stmt->execute();
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC); error_log("result doAction :".print_r($result, 1));

            if(empty($result['requis'])){  error_log("result requis nul :".$result['requis']);
                $stmt = $this->_dbh->prepare("UPDATE action SET status = 1 WHERE map_id = $this->map");
                $stmt->execute();
            }
            ...
            if(!empty($result['requis']) && $_SESSION['inventaire'] != ""){
            $stmt = $this->_dbh->prepare("UPDATE action SET status = 1 WHERE map_id = $this->map");
            $stmt->execute();
            }
