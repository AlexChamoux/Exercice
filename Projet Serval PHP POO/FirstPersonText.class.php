...
class FirstPersonText extends BaseClass{
    ...
    public function getText(BaseClass $Base){    
            ...
            $stmt = $this->_dbh->prepare("SELECT * FROM action WHERE map_id = $this->map");
            $stmt->execute();
            $response = $stmt->fetch(PDO::FETCH_ASSOC); error_log("response texte :".print_r($response, 1));

            if(!empty($response['status'])){
                $status_action = $reponse['status'];
            }else{
                $status_action = 0;
            }

            $stmt = $this->_dbh->prepare("SELECT * FROM text WHERE map_id = $this->map AND status_action = $status_action ");
            $stmt->execute();
